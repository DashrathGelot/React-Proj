    ...
    const allTaskShowData=this.state.allTask.map(task=> {
      if(this.state.edit===task){
        return(
          <tr>
          <td>
          <form onSubmit={this.onSubmitUpdateHandle.bind(this)}>
            <input type="text" name="task" value={this.state.taskName} onChange={(e)=>{this.setState({taskName:e.target.value})}} />
            <input type="submit" value="Update"/>
          </form>
          </td>
          <td><button value={task} onClick={this.deleteTask}>Delete</button></td>
          </tr>
        );
      }
      else{
        return(
          <tr>
              ...
              <td><button value={task} onClick={this.updateTask}>Update</button></td>
              <td><button value={task} onClick={this.deleteTask}>Delete</button></td>
          </tr>
        );
      }
